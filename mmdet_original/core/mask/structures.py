...
class BitmapMasks(BaseInstanceMasks):
    ...
    def __init__(self, masks, height, width):
        ...
        if len(masks) == 0:
            ...
            #     self.masks = np.stack(masks).reshape(-1, height, width)
            # except:
            #     print(f'now array: {masks.shape[1:]}, demand: {height}, {width}')

            # try:  # reshape이 되야지 가능함.
            #     assert self.masks.shape[1] == self.height
            #     assert self.masks.shape[2] == self.width
            # except:
            #     print(f'now array: {masks}')
    ...
    def pad(self, out_shape, pad_val=0):
        ...
        if len(self.masks) == 0:
            ...
            # print(f'padded_mask {padded_masks.shape} out_shape{out_shape}')
...
class PolygonMasks(BaseInstanceMasks):
    ...
    def __getitem__(self, index):
        ...
        if isinstance(index, list):
            masks = [self.masks[i] for i in index]  # debug1: masks는 분명 존재하는데 index가 []이라 [[]]이 된듯
        else:
            try:
