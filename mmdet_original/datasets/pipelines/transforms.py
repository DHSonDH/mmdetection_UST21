...
class Pad:
    ...
    def _pad_img(self, results):
        ...
        for key in results.get('img_fields', ['img']):
            ...
            if self.size is not None:
                ...
                # 여기서 shape이 바뀜. keep_ratio=True하면 img_shape은 가로세로 대칭이 아닌데, padded_img는 가로세로 대칭으로 나옴
                # 일단은 Keep_ratio = False해서 정사각 이미지로 처리함
    ...
    def __call__(self, results):
        ...
        """
        self._pad_img(results)   # 이거 전 후 pad_shape이 바뀌는지?
    ...
    def _crop_data(self, results, crop_size, allow_negative_crop):
        ...
        for key in results.get('bbox_fields', []):
            ...
            if (key == 'gt_bboxes' and not valid_inds.any()
                ...
                return None
            results[key] = bboxes[valid_inds, :]  # debug2 : valid_inds.nonzero()[0]가 index인데 []이 되버림
            # label fields. e.g. gt_labels and gt_labels_ignore
            label_key = self.bbox2label.get(key)
            if label_key in results:
                results[label_key] = results[label_key][valid_inds]
            ...
            if mask_key in results:
                results[mask_key] = results[mask_key][   #!!! 왜 mask가 갑자기 [[]]이 되는지? 애초에 warning 무시해서 그런듯... 
                    valid_inds.nonzero()[0]].crop(  # debug2 : valid_inds.nonzero()[0]가 index인데 []이 되버림
